...
//idpair idk=idpair(1,0);
namespace cslam {

Communicator::Communicator(ccptr pCC, vocptr pVoc, mapptr pMap, dbptr pKFDB ,pcmptr pPointCloud,bool bLoadedMap)
    : mpCC(pCC),
      ...
      mnEmptyMsgs(0),mpPointCloudMapping(pPointCloud),
    ...
    while (!mlBufKFin.empty() && (ItCount < mKfItBound))
    {
        ...
        if(msgpair.first.mClientId != MAPRANGE)
        {
            ...
            *pMsg = msgpair.first;
            ...
            #endif
            for(int i = 0; i <pMsg->mPointCloud.size(); i++){
                    pKF->passDepth.push_back(pMsg->mPointCloud[i].mDepth);
                    pKF->passx.push_back(pMsg->mPointCloud[i].mx);
                    pKF->passy.push_back(pMsg->mPointCloud[i].my);
                    pKF->passblue.push_back(pMsg->mPointCloud[i].mblue);
                    pKF->passgreen.push_back(pMsg->mPointCloud[i].mgreen);
                    pKF->passred.push_back(pMsg->mPointCloud[i].mred);
                }
            pKF->mb=pMsg->mb;
            pKF->mbf=pMsg->mbf;
            for(int i=0;i<pMsg->mright.size();i++){
                pKF->mvuRight.push_back(pMsg->mright[i].mvuRight);
            }
            ...
            ///my add
             ...
             mpPointCloudMapping->insertKeyFrame(pKF, pKF->mId);
